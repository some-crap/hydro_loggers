...
class SensorDataProcessorApp:
    ...
    def create_widgets(self):
        ...
        label_dev_eui = tk.Label(frame, text="ID датчика (если не найден Config.txt):")
    ...
    def extract_dev_eui(self, config_file_path):
        try:
    ...
    def process_gorizont_files(self):
        ...
        # Найти файл Config.txt
        config_file_path = os.path.join(self.folder_path, 'Config.txt')
        if os.path.exists(config_file_path):
            self.dev_eui = self.extract_dev_eui(config_file_path)
            if not self.dev_eui:
    ...
    def process_files(self):
        ...
        if sensor_type == "GORIZONT":
            ...
            # Логика для PROMODEM будет добавлена позже
            messagebox.showinfo("PROMODEM", "Обработка для PROMODEM будет добавлена позже.")
        else:
